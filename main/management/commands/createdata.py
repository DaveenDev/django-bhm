...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        self.stdout.write(self.style.SUCCESS(f"Number of Suppliers: { suppliers_count }"))
        
        # GENERATE RANDOM LOCATION
        for _ in range(2):            
            InvLocation.objects.create(
                name = fake.unique.getRandomLocation()
            )

        locations = InvLocation.objects.all()
        locations_count =locations.count()
        locations_ids = locations.values_list('id', flat=True)
        self.stdout.write(self.style.SUCCESS(f"Number of Locations: { locations_count }"))

         # GENERATE RANDOM UNITS
        for _ in range(4):            
            Unit.objects.create(
                unit = fake.unique.getRandomUnit()
            )
        
        # GENERATE RANDOM CATEGORY
        for _ in range(13):
            category_name = fake.unique.getRandomCategory()
        ...
        self.stdout.write(self.style.SUCCESS(f"Number of Categories: {category_count}"))

        # GENERATE RANDOM PRODUCT
        for _ in range(1000):
            prod_code = fake.bothify('###', letters='ABCDE') + str(fake.pyint())
